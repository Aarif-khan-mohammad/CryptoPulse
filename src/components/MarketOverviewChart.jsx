...
import { BarChart2, TrendingUp, TrendingDown, RefreshCw, AlertCircle } from 'lucide-react'
import {
...
export default function MarketOverviewChart() {
  ...
  const { data, loading, error } = useGlobalChart(range)
    ...
    >
      ...
      {loading && (
        <div className="h-44 flex flex-col items-center justify-center gap-2">
          ...
          <span className="text-slate-600 text-xs">Loading chart data...</span>
        </div>
      )}
      ...
      {!loading && error && (
        <div className="h-44 flex flex-col items-center justify-center gap-2 text-slate-500">
          ...
          <span className="text-xs">Chart unavailable — rate limit reached</span>
          <span className="text-[10px] text-slate-600">Will retry on next page load</span>
        </div>
      )}
        ...
        <div className="h-44">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              ...
              <YAxis yAxisId="price" orientation="right" tick={{ fill: '#64748b', fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} width={44} domain={['auto', 'auto']} />
              <YAxis yAxisId="vol"   orientation="left"  tick={{ fill: '#64748b', fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={fmtK} width={52} />
              ...
              <Area  yAxisId="price" type="monotone" dataKey="btc" name="BTC Price" stroke={isUp ? '#00ff88' : '#e11d48'} strokeWidth={2} fill="url(#btcGrad)" dot={false} isAnimationActive={false} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      )}
